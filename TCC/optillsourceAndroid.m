function [ x_opt, fval, history ] = optillsourceAndroid( method, eigenvalue, eigenfunction, OPTparams, object )
% optimizes a light source to maximize i.e. phase-contrast for a given object 
% Written by Benedict Diederich, benedict.diederich@leibniz-ipht.de
% www.nanoimaging.de
% License: GPL v3 or later.
%
% function [ x_opt, fval, history ] = optillsourceAndroid( method, eigenvalue, eigenfunction, OPTparams, object )
% inputs: method - illumination source-shape (i.e. Zernike, Segments)
%       eigenvalue 
%       eigenfunction
%       OPTparams
%       object - 2D complex transmisison functions
% outputs:
%       x_opt - parameters which then form a lightsource
%       fval - 
%       history - 
...
if( strcmp(OPTparams.optmethod, 'swarm'))
    ...
    fun = @psobjTCC;
    objective = @(x)fun(x, xmask, objectspectrum, eigenfunction, eigenvalue, kernelOrder, object, OPTparams.qualitymethod);
